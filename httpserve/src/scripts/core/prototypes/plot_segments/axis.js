...
var AxisRenderer = /** @class */ (function () {
    ...
    AxisRenderer.prototype.renderLine = function (x, y, angle, side, axisOffset) {
        ...
        // Ticks
        var ticksData = this.ticks.map(function (x) { return x.position; });
        var visibleTicks = ticksData.concat([rangeMin, rangeMax]);
        if (style.showTicks) {
            try {
        ...
        return [
            manager.vertical(manager.verticalGroup({
                ...
                manager.inputColor({
                    ...
                    labelKey: strings_1.strings.objects.axes.tickTextBackgroudColor,
                    allowNull: true,
                }),
                manager.inputNumber({
                    property: axisProperty,
...
function buildScrollingAxisWidgets(data, axisProperty, manager, axisName, onChange) {
    return [
...
function buildAxisWidgets(data, axisProperty, manager, axisName, axisWidgetsConfig, onChange) {
    ...
    if (data != null) {
        ...
        var scrollingWidgets = axisWidgetsConfig.showScrolling
            ? buildScrollingAxisWidgets(data, axisProperty, manager, axisName, onChange)
            : [];
        var tickFormatAndTickDataFields = getTickDataAndTickFormatFields(data, axisProperty, manager);
        ...
        switch (data.type) {
            ...
            case "default":
                {
                    widgets.push(manager.sectionHeader(axisName + strings_1.strings.objects.axes.stackingSuffix, manager.clearButton({ property: axisProperty }, null, true), __assign(__assign({}, dropzoneOptions), { noLineHeight: false })), manager.inputNumber({ property: axisProperty, field: "gapRatio" }, {
                        minimum: 0,
                        maximum: 1,
                        percentage: true,
                        showSlider: true,
...
function getOrderByAnotherColumnWidgets(data, axisProperty, manager) {
    var _a, _b;
    var widgets = [];
    ...
    widgets.push(manager.label(strings_1.strings.objects.axes.orderBy), manager.horizontal([1, 0], manager.inputSelect({ property: axisProperty, field: "orderByExpression" }, {
        type: "dropdown",
        showLabel: true,
...
function getTickDataAndTickFormatFields(data, axisProperty, manager) {
    var showInputFormat = utils_1.shouldShowTickFormatForTickExpression(data, manager);
    var widgets = [];
    widgets.push(manager.label(strings_1.strings.objects.axes.tickData), manager.styledHorizontal({
        alignItems: "start",
