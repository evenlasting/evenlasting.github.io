...
exports.VirtualScrollBar = function (_a) {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("g", { className: "controls" },
            React.createElement("rect", { ref: track, x: Math.min(x, x + width) + (vertical ? 0 : buttonsWidth), y: -Math.max(y, y + height) + (vertical ? buttonsHeight : 0), width: Math.abs(width) - (vertical ? 0 : buttonsWidth * 2), height: Math.abs(height) - (vertical ? buttonsHeight * 2 : 0), onMouseUp: function () {
                    setActive(false);
                }, style: {
                    fill: "#e1e1e1",
                    opacity: 1,
