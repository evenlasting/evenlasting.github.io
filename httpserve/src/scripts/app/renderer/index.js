        ...
        case "text": {
            ...
            if (text.style.backgroundColor) {
                return (React.createElement("filter", { x: "0", y: "0", width: "1", height: "1", id: text.style.backgroundColorId },
                    React.createElement("feFlood", { floodColor: renderColor(text.style.backgroundColor, text.style.colorFilter), result: "bg" }),
...
function renderGraphicalElementSVG(element, options) {
    ...
    switch (element.type) {
        ...
        case "path": {
            ...
            return (React.createElement("path", __assign({ key: options.key }, mouseEvents, { className: options.className || null, style: style, d: d, transform: path.transform })));
        }
        case "text-on-path": {
            var text = element;
